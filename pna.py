...
from torch_geometric.nn import PNAConv, Linear
from configure import hic_edge_intra, hic_edge_inter, string_edge_file, \
    ...
    X_masked_path, y_path, X_path, X_val_masked_path, X_val_path,\
    y_val_path, pna_model_path, pna_model_name, pna_model_masked_name,\
        mask_exp, pna_y_pred_path, pna_y_pred_path_masked
...
print(f"edges selected: {choices}")
...
def save_model_path():
    folder = pna_model_path
    full = f"{folder}/{pna_model_masked_name}" if is_masked else f"{folder}/{pna_model_name}"
    return full
...
print(f"total_edges :{edge_index.shape[1]}")
# get the in-degree histogram
hist, _ = np.histogram(edge_index[1])
...
class Net(torch.nn.Module):
    def __init__(self, y_dim):
        ...
        self.conv1 = PNAConv(1, 8, \
                             aggregators=["mean", "max", "min", "std"], \
                                scalers=["linear", "identity", "amplification", "attenuation"], \
                                     deg=hist )                                
...
if __name__ == '__main__':
        ...
        for epoch in range(num_epochs):
            ...
            for i, data in pbar:
                ...
                info = f'm:{is_masked}, c:{str(choices)}, epoch: {epoch}, epoch_loss_avg: {epoch_loss_avg}'
...
if validate:
    ...
    out_path = pna_y_pred_path_masked if is_masked else pna_y_pred_path 
